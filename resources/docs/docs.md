...
The `home-routes` handler in the `todoapp.routes.home` namespace
...
The `home-page` function will in turn call the `todoapp.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `todoapp.handler` namespace:
...
Request middleware functions are located under the `todoapp.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `todoapp.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `todoapp.db.core` in some other namespace.
